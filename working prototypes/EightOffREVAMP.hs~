...
eOExpt seed = (winCount, average)
    where
        scores = [eOGame ( eODeal x) | x <- [seed..(seed + 100)]]
        winCount = length $ filter (== 52) scores
        average = (fromIntegral (foldr (+) 0 scores)) / (fromIntegral 100)
