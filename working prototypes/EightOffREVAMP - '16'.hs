...
getSequenceMove board@(f, cols, res) card
    ...
    | elem card res = Just ((f, addToColumns cols [card], delete card res), -10 - kValue - cValue) -- The needed card is in the reserves, pull it out and add it to the columns
    ...
    | length headToCard == length seq && length seq < maxSeqLen = Just ((f, addToColumns (map (\\ seq) cols) seq, res), -1 * (length seq) - kValue - hValue)
    ...
    where
        ...
        cValue = if elem card bonusCards then 100 else 0 -- Rank modifier for bonus cards (singles)
        ...
        seq = reverse (getCardSequence (reverse headToCard)) -- The sequence of cards to be placed, the head being the lowest in sequence
...
-- getCardSequence takes a list of cards and returns all cards (starting from the head) that are predecessors of the previous card
getCardSequence :: [Card] -> [Card]
getCardSequence [] = [] -- If no list was given, return a null list
getCardSequence (h:t)
    ...
    | head t == pCard h = h:(getCardSequence t) -- If the head of the tail of the list is the predecessor of this card, append this head and run recursively
