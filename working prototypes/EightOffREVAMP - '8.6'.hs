...
-- TODO: Change the 0 on the line below (and similar above) to -100?
    | isKing card && length cols == 8 = Nothing -- If the card is a king, then only proceed if there is somewhere to put the new column it will for
    | elem card res = Just ((f, addToColumns cols [card], delete card res), 0 - kValue) -- The needed card is in the reserves, this move pulls it out and adds it to the columns
    ...
    | length headToCard == length sequence && length sequence < maxSeqLen = Just ((f, addToColumns (map (\\ sequence) cols) sequence, res), -1 * (length sequence) - kValue)
    ...
    -- If it's in a column but covered, make the move to uncover it (if there's a free res slot and no chain is broken) and give it a cost as the number of moves it would take to uncover it:
    | length res < 8 && nonBroken = Just ((f, map (delete topCard) cols, topCard:res), length headToCard - length sequence - kValue)
    -- If none of the above, then it's in a column, but there are no moves to put it anywhere, so return Nothing.
    | otherwise = Nothing
    where
        kValue = if isKing card then 100 else 0 -- If the move is a king move, then give it priority
...
addToColumns cols seq
    | isKing (head (reverse seq)) = seq:populatedCols
    | otherwise = map (insertSequence seq) populatedCols
    where
