{- |
Module      :  EightOff
Description :  An implementation of an EightOff game, including basic AI. Please see from line 98 onward for Assignment 3 work.
...
findMoves board@(f, cols, res) = map fst (sortBy (\(_,n1) (_,n2) -> compare n1 n2) unsortedMoves) -- Sort all potential moves by rank
    where
        ...
        unsortedMoves = map resMaybe $ filter isJust $ map (getSequenceMove board) (neededCards ++ neededKings) -- List of possible moves, with importance rank
...
getSequenceMove board@(f, cols, res) card
    ...
    | elem card res = Just ((f, addToColumns cols [card], delete card res), -10 - kValue - bValue) -- The needed card is in the reserves, pull it out and add it to the columns
    ...
    | length headToCard == length seq && length seq <= resSpace + 1 = Just ((f, addToColumns (map (\\ seq) cols) seq, res), -1 * (length seq) - kValue - hValue)
    ...
    | length coveringSeq <= resSpace = Just ((f, map (\\ coveringSeq) cols, res ++ coveringSeq), length headToCard - length seq - kValue - hValue)
   -- | length res < 8 && nonBroken = Just ((f, map (delete topCard) cols, topCard:res), length headToCard - length seq - kValue - hValue)
    ...
    where
        ...
        hValue = if elem (head seq) bonusCards then 100 else 0 -- Rank modifier for bonus cards (at the end of sequences)
        bValue = if elem card bonusCards then 100 else 0 -- Rank modifier for bonus cards (singles)
        kValue = if isKing card then 100 else 0 -- Rank modifier for creation of a new column (king moves)
...
chooseMove board
    ...
    where moves = findMoves board -- The list of all moves

-- eOGame takes an initial board, then plays the game, returning a final score
eOGame :: EOBoard -> Int
eOGame board
    | isJust nextMove = eOGame(resMaybe nextMove) -- If there is a next move, play it with the updated board state
...
-- eOExpt takes an integer and runs 100 games with consecutive games being played using consecutive integers after the given one for seeds
-- Returns a tuple consisting of the number of wins in 100 games and the average score
eOExpt :: Int -> (Int, Double)
eOExpt seed = (winCount, average) -- Return the appropriate information
    where
        scores = [eOGame ( eODeal x) | x <- [seed..(seed + 100)]] -- Store all game outcomes
        winCount = length $ filter (== 52) scores -- Find the number of won games
        average = (fromIntegral (foldr (+) 0 scores)) / (fromIntegral 100) -- Calculate the average score
