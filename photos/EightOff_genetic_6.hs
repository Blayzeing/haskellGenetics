{- Implemention for use by genetic algorithm to find optimum play strategy.
 - This system requires a genetic sequence of length 6.
 -}
...
getSequenceMove gene board@(f, cols, res) card
    ...
    | length headToCard == length seq && length seq <= resSpace + 1 = Just ((f, addToColumns (map (\\ seq) cols) seq, res), (gene!!1) * fromIntegral(length seq) - kValue - hValue)
    ...
    where
        ...
        hValue = if elem (head seq) bonusCards then gene!!3 else 0 -- Rank modifier for bonus cards (at the end of sequences)
        bValue = if elem card bonusCards then gene!!4 else 0 -- Rank modifier for bonus cards (singles)
        kValue = if isKing card then gene!!5 else 0 -- Rank modifier for creation of a new column (king moves)
...
eOExpt seed gene = (winCount, average) -- Return the appropriate information
    where
        scores = [eOGame gene ( eODeal x) | x <- [seed..(seed + 100)]] -- Store all game outcomes
...
eOExptSingle seed gene = fromIntegral winCount -- Return the appropriate information
    where
        scores = [eOGame gene ( eODeal x) | x <- [seed..(seed + 100)]] -- Store all game outcomes
        winCount = length $ filter (== 52) scores -- Find the number of won games
